...
class KinesisFlowSpec extends WordSpecLike with Matchers with KinesisMock {
  ...
  "KinesisFlow" must {
    ...
    "publish records with retries" in assertAllStagesStopped {
      new DefaultSettings with KinesisFlowProbe with WithPutRecordsInitialErrorsSuccessfulRetry {
        ...
        sinkProbe.requestNext(settings.retryInitialTimeout * 1) shouldBe publishedRecord
      ...
      .map(
        i =>
          (PutRecordsRequestEntry
             .builder()
             .partitionKey("partition-key")
             .data(SdkBytes.fromUtf8String(i.toString))
             .build(),
           i)
      )
  ...
  trait WithPutRecordsSuccess { self: Settings =>
    ...
    when(kinesisAsyncClient.putRecords(PutRecordsRequest.builder().build())).thenAnswer(new Answer[AnyRef] {
  ...
  trait WithPutRecordsWithPartialErrors { self: Settings =>
    val failingRecord = PutRecordsResultEntry
      .builder()
      .errorCode("error-code")
      .errorMessage("error-message")
      .build()
    when(kinesisAsyncClient.putRecords(PutRecordsRequest.builder().build()))
